...
public class BanHistoryCommand implements CommandExecutor {
    ...
    public BanHistoryCommand(CyberBanPlugin plugin) { this.plugin = plugin; }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length != 1) { sender.sendMessage(plugin.getMessageUtil().get("usage-banhistory")); return true; }
